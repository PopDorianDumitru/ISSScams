...
CREATE TABLE BenignFlaggedCrossedWithCompany (
	...
	CONSTRAINT benignusersFK FOREIGN KEY(mockuser_id) REFERENCES BenignFlaggedUserIDs(mockuser_id) 
		ON DELETE CASCADE 
		ON UPDATE CASCADE,
	CONSTRAINT benigncompaniesFK FOREIGN KEY(company_id) REFERENCES CompanyTokens(company_id)
		ON DELETE CASCADE 
		ON UPDATE CASCADE
);
...
CREATE TABLE SevereFlaggedCrossedWithCompany (
	...
	company_id INT NOT NULL,
	PRIMARY KEY(mockuser_id, company_id),
	CONSTRAINT severeusersFK FOREIGN KEY(mockuser_id) REFERENCES SevereFlaggedUserIDs(mockuser_id) 
		ON DELETE CASCADE 
		ON UPDATE CASCADE,
	CONSTRAINT severecompaniesFK FOREIGN KEY(company_id) REFERENCES CompanyTokens(company_id)
		ON DELETE CASCADE 
		ON UPDATE CASCADE
);

-- eventual insert data